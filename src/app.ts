...
app.get(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      await AppDataSource.manager.query(
        `DELETE FROM memberof WHERE StudentID = ?`,
        [studentId]
      );
      ...
      if (test === "A") {
        ...
        output = `Case A: Successfully inserted two departments (1 & 2) for student ${studentId}.`;
        ...
        for (const deptId of [1, 2]) {
          await AppDataSource.manager.query(
            `INSERT IGNORE INTO memberof (StudentID, DepartmentID) VALUES (?, ?)`,
            [studentId, deptId]
          );
        }
        // now try the third
...
app.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      // 3) Respond with JSON
      res.json({ studentId: stuId, averageGrade: average });
      return;
    } catch (err: any) {
...
app.get(
  ...
  async (req: Request, res: Response):Promise<void>=> {
    ...
    try {
      ...
      await AppDataSource.manager.query(
        `CALL CalculateCourseAvgGrade(?, @c_avg);`,
        [crn]
      );

      // 2) Retrieve the OUT parameter
      const [[{ "@c_avg": courseAvg }]]: any = await AppDataSource.manager.query(
        `SELECT @c_avg;`
      );

      // 3) Respond with JSON
      res.json({ crn, courseAvg });
      return;
    } catch (err: any) {
...
app.get(
  ...
  async (req: Request, res: Response):Promise<void> => {
    ...
    try {
      ...
      // 2) Retrieve the OUT parameter
      const [[{ "@total_credit": totalCredit }]]: any =
        await AppDataSource.manager.query(`SELECT @total_credit;`);

      // 3) Respond with JSON
      res.json({ studentId: stuId, totalCredit });
      return;
    } catch (err: any) {
